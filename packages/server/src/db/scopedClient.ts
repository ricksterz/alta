...
//
// Strategy:
//  - Every read/bulk-write operation gets `tenantId` merged into its
//    `where` automatically.
//  - `create`/`createMany` get `tenantId` merged into `data` automatically.
...
//    add a non-unique tenantId filter to those, so allowing them through
//    unscoped would be a silent cross-tenant read/write hole. Use the
//    *Many equivalent with an explicit { id, tenantId } filter instead.
//  - Tenant itself is exempt (it has no tenantId column — it IS the tenant).
          ...
          if (BANNED_OPERATIONS.has(operation)) {
            throw new Error(
              `${model}.${operation} is disabled on the tenant-scoped client: its ` +
                ...
                `{ id, tenantId } filter, or findFirst, instead.`
            );
          }
