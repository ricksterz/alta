...
const DOMAIN_STYLES: Record<string, string> = {
  ...
  investor: "bg-sky-50 dark:bg-sky-950 text-sky-700 dark:text-sky-300",
  fund: "bg-violet-50 dark:bg-violet-950 text-violet-700 dark:text-violet-300",
  template: "bg-violet-50 dark:bg-violet-950 text-violet-700 dark:text-violet-300",
  subscription: "bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300",
  capital_call: "bg-amber-50 dark:bg-amber-950 text-amber-700 dark:text-amber-300",
...
function ActionBadge({ action }: { action: string }) {
  ...
  const style = DOMAIN_STYLES[domain] ?? "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400";
  return (
  ...
  return (
    <div>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="text-left text-xs text-slate-500 dark:text-slate-400 hover:text-slate-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
      >
        {open ? "Hide" : preview}
        {!open && entries.length > 2 && ` · +${entries.length - 2}`}
      </button>
      {open && (
        <pre className="mt-1 max-w-md overflow-x-auto rounded bg-slate-50 dark:bg-slate-800 p-2 text-xs text-slate-700 dark:text-slate-300">
...
export function AuditPage() {
  ...
  const inputClass =
    "rounded border border-slate-300 dark:border-slate-700 px-3 py-2 text-sm focus:border-slate-500 dark:focus:border-slate-400 focus:outline-none";
      ...
      <div className="mb-5 flex flex-wrap gap-3">
        ...
        {(action || entityType || search || from) && (
          <button
            ...
            className="rounded border border-slate-300 dark:border-slate-700 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
          >
            Clear
          </button>
        )}
        ...
        <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {events.map((e) => (
                <tr key={e.id} className="align-top hover:bg-slate-50 dark:hover:bg-slate-800">
                  ...
                  <td className="px-4 py-3">
                    <div className="text-slate-800 dark:text-slate-200">{e.entityType}</div>
                  ...
                  <td className="px-4 py-3">
                    {e.actor ? (
                      <>
                        ...
                        <div className="text-xs text-slate-400 dark:text-slate-500">{e.actor.email}</div>
                      </>
                    ) : (
                      <span className="text-slate-400 dark:text-slate-500">{e.actorType}</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
      ...
      {nextCursor && (
        <div className="mt-4 flex justify-center">
          <button
            ...
            className="rounded border border-slate-300 dark:border-slate-700 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-40"
          >
